...
class TripletLoss(nn.Module):
    ...
    def __init__(self, margin=0.5, normalize_feature=False):
...
class CycleLoss(nn.Module):
    ...
    def forward(self, emb, adj_similarity):
        if self.normalize_feature:
            # equal to cosine similarity
            emb = F.normalize(emb)
        mat_dist = euclidean_dist(emb, emb)
        ...
        mat_dist = F.softmax(mat_dist/0.001, dim=-1)
        # loss1 = torch.abs(mat_dist - adj_similarity*2.5)
        loss1 = F.relu(mat_dist.mul(adj_similarity) + 0.001*(1-adj_similarity)-mat_dist.mul((1-adj_similarity)))
        # loss1 = F.relu(mat_dist.mul(adj_similarity))


        loss1 = loss1.sum().sum()/(n*n)
        prec = 1
        return loss1, prec
