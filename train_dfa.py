...
from utils import *
from loss import OriTripletLoss, TripletLoss, CycleLoss
...
parser.add_argument('--log_path', default='log/', type=str,
                    help='log save path')
parser.add_argument('--vis_log_path', default='log/vis_log_dfa/', type=str,
                    help='log save path')
...
criterion3 = TripletLoss(margin=args.margin)
criterion2 = CycleLoss(margin=args.margin)
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        w_norm = adj.pow(2).sum(1, keepdim=True).pow(1. / 2)  # sum(1) 按行求和
        adj_norm = adj.div(w_norm) # normalized adjacency matrix 
 
        ####################################
        adj_similarity = torch.mm(one_hot, torch.transpose(one_hot, 0, 1)).float() #+ torch.eye(labels.size()[0]).float()
        ...
        adj_similarity = Variable(adj_similarity.cuda())
        
        data_time.update(time.time() - end)

        # Forward into the network
        feat, part_refined, out0, out_att = net(input1, input2, adj_norm)

        # baseline loss: identity loss + triplet loss Eq. (1)
        loss_id = criterion1(out0, labels)
        loss_tri_1, batch_acc = criterion3(feat, labels)

        # prototype learning
        loss_tri = criterion2(feat, adj_similarity)[0]
        ...
        loss = loss_id + loss_tri_1 + 1.5*loss_tri + 1.1*loss_p   # 
        ...
        tri_1_loss.update(loss_tri_1.item(), 2 * input1.size(0))
        id_loss.update(loss_id.item(), 2 * input1.size(0))
        tri_loss.update(loss_tri.item(), 2 * input1.size(0))
        p_loss.update(loss_p.item(), 2 * input1.size(0))
        total += labels.size(0)
        ...
        if batch_idx % 10 == 0:
            print('Epoch: [{}][{}/{}] '
                  ...
                  'T_1Loss: {tri_1_loss.val:.4f} ({tri_1_loss.avg:.4f}) '
                  'TLoss: {tri_loss.val:.4f} ({tri_loss.avg:.4f}) '
                  'pLoss: {p_loss.val:.4f} ({p_loss.avg:.4f}) '
                  'Accu: {:.2f}'.format(
                   epoch, batch_idx, len(trainloader), current_lr,
                   100. * correct / total, batch_time=batch_time,
                   train_loss=train_loss, id_loss=id_loss, tri_1_loss=tri_1_loss, tri_loss=tri_loss, p_loss=p_loss))
